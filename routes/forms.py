...
class AddAirportForm(forms.Form):
    ...
    parent = forms.ModelChoiceField(
        ...
        required=False,
        label="Parent Airport (leave empty for main airport)",
    )
    ...
    position = forms.ChoiceField(
        choices=POSITION_CHOICES, required=False, label="Position (Left or Right)"
    )

    # Airport code (must be unique)
    code = forms.CharField(max_length=10, label="Airport Code (e.g. JFK)")

    # Duration in minutes
    duration = forms.IntegerField(min_value=0, label="Duration (in minutes)")

    # Custom validation for airport code
    def clean_code(self):
    ...
    def clean(self):
        ...
        if parent:
            if position == "left" and parent.left:
                raise forms.ValidationError(f"{parent.code} already has a left child.")

            if position == "right" and parent.right:
                raise forms.ValidationError(f"{parent.code} already has a right child.")
...
class SearchForm(forms.Form):
    ...
    start_airport = forms.ModelChoiceField(
        queryset=Airport.objects.all(), label="Starting Airport"
    )
    ...
    direction = forms.ChoiceField(
        choices=DIRECTION_CHOICES, label="Search Direction (Left or Right)"
    )
